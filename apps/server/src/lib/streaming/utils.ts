...
export function aiSdkProviderOptions(
  ...
  // Single standard language selected (e.g. ['es'], ['fr'], ['ja'], ['de'])
  if (codes.length === 1 && codes[0] !== "hinglish") {
    const key = LANGUAGE_OPTION_KEYS[providerId] ?? "language";
    options[providerId] = { ...options[providerId], [key]: codes[0] };
    return options;
  }

  // Multi-language selected pool or Hinglish (e.g. ['en', 'es'], ['fr', 'de'], ['ja', 'en'], ['en', 'hinglish'])
  // Leave STT unconstrained so code-switching works naturally and Whisper doesn't hallucinate prompt text on silence.
  return Object.keys(options).length > 0 ? options : undefined;
}
