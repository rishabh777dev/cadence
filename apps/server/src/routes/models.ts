...
const models = new Hono()
  ...
  .get("/live/:provider", async (c) => {
    const provider = c.req.param("provider");
    const apiKey = c.req.query("apiKey");

    try {
      const models = await getLiveProviderModels(provider, apiKey);
      return c.json({ provider, models });
    } catch (err) {
      return c.json(
        {
          error: `Failed to fetch live models for ${provider}`,
          detail: String(err),
          models: [],
        },
        500,
  ...
  .post("/test", async (c) => {
    ...
    const modelId = body.model_id?.trim() ?? "default";
    const type = body.type ?? "llm";
    ...
    try {
      if (type === "voice") {
        if (provider === "local-whisper") {
          ...
          if (!status || status.status !== "ready") {
            return c.json({
              ok: false,
              error: `Local Whisper model "${modelId}" is not downloaded or ready.`,
            });
          }
          return c.json({
            ok: true,
            latencyMs: Date.now() - start,
            message: `Local Whisper model is ready on-device.`,
          });
        }

        const apiKey = body.api_key || getApiKeyForProvider(provider);
        if (!apiKey && provider !== "freestyle-cloud") {
          return c.json({
            ok: false,
        ...
        return c.json({
          ok: true,
          latencyMs: Date.now() - start,
          message: `${provider} voice endpoint verified.`,
        });
      }

      // LLM probe test
      const chatModel = await createChatModel(provider, modelId);
      ...
      return c.json({
        ...
        message: `Model responded in ${latencyMs}ms.`,
      });
    } catch (err) {
