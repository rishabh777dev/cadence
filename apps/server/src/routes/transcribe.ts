...
import { CloudAuthError } from "../lib/streaming/providers/cadence-cloud.js";
...
const transcribeRoute = new Hono().post("/", async (c) => {
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        if (err instanceof FreestyleCloudUsageError) {
          return c.json(
            { error: "usage_exceeded", resetsAt: err.resetsAt },
            429,
          );
        }
        log.error(
    ...
    return c.json({ error: message || "Transcription failed" }, 500);
  }
});
