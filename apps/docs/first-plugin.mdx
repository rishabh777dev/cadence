...
```json
{
  ...
  "dependencies": { "cadence-voice": "workspace:*" }
}
```
...
<Tip>
  ...
  ```ts
  import { transform } from "cadence-voice";
...
```json
["@cadence-voice/profanity-filter", { "preserveCase": true }]
```
...
```ts
import type { FreestyleBridge } from "cadence-voice";
