...
const api = {
  ...
  defaultMagicEditHotkey: getDefaultMagicEditHotkey(),
  ...
  updateMagicEditHotkey: (hotkey: string): void =>
    ipcRenderer.send("magic-edit-hotkey:update", hotkey),
  ...
  onMagicEditDown: (callback: (selectedText: string) => void): (() => void) => {
    const handler = (_event: unknown, selectedText: string): void =>
      callback(selectedText);
    ipcRenderer.on("magic-edit:down", handler);
    return () => ipcRenderer.removeListener("magic-edit:down", handler);
  },
  onMagicEditUp: (callback: () => void): (() => void) => {
    const handler = (): void => callback();
    ipcRenderer.on("magic-edit:up", handler);
    return () => ipcRenderer.removeListener("magic-edit:up", handler);
  },
