...
// ---------------------------------------------------------------------------
// Filter bar — source & provider chips with active key indicator dots
// ---------------------------------------------------------------------------
...
function FilterBar({
  ...
  const allFilters = [...baseSources, ...providerSources];

  return (
    <div className="border-border flex flex-wrap items-center gap-1.5 border-b px-5 py-2.5 bg-card/40">
      {allFilters.map((f) => {
        const hasKey =
          f.id !== "all" && f.id !== "local" && keyProviders.has(f.id);
        return (
          <Chip
            key={f.id}
            label={f.label}
...
function Chip({
  ...
  return (
    <Button
      ...
      onClick={onClick}
      className={cn(
        "rounded-full text-xs font-medium transition-all gap-1 h-6 px-2.5 cursor-pointer",
        on
          ? "shadow-sm"
          : "bg-background/60 hover:bg-secondary/80 text-muted-foreground hover:text-foreground",
      )}
    >
      <span>{label}</span>
      {hasKey && (
        <span
          className="size-1.5 rounded-full bg-emerald-400 inline-block shrink-0"
...
function EndpointConnectForm({
  ...
  if (!isOpen) {
    return (
      <div className="border-border border-b px-5 py-2.5 bg-muted/20 flex items-center justify-between">
        <span className="text-muted-foreground text-xs font-medium">
          Need a custom self-hosted server (vLLM, Ollama, LiteLLM)?
        </span>
        <Button
          variant="outline"
          size="xs"
          onClick={() => setIsOpen(true)}
          className="text-xs h-6 px-2.5 rounded-lg cursor-pointer"
        >
          ⚙️ Custom Endpoint…
        </Button>
      </div>
  ...
  return (
    <div className="border-border border-b px-5 py-4 bg-muted/30">
