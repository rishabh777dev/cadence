...
const audioPlaybackOptions = [
  ...
  { id: "duck", label: "Lower volume", icon: Volume2 },
  { id: "pause", label: "Pause playback", icon: Pause },
] as const;
...
export default function SettingsPage(): React.JSX.Element {
  ...
  } = useHotkeyRecorder(handleHotkeyRecorded, { target: "dictation" });
        ...
        <div className="min-h-0 overflow-y-auto px-1 -mx-1">
          ...
          {activeSection === "application" && (
            <SettingsPanel>
              ...
              >
                <div className="flex items-center justify-between w-full max-w-md gap-3 bg-secondary/40 border border-border/60 rounded-xl px-4 py-2.5">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="rounded-lg h-8 px-3 text-xs font-semibold shrink-0 cursor-pointer"
            ...
            <SettingsPanel>
              ...
              {supportsBackgroundAudio ? (
                <Row
                  ...
                  desc="Lower volume automatically softens background audio while dictating. Pause playback temporarily pauses music and resumes when you finish."
                  last
                >
                  <Segment
                    compact
      ...
      {settingsSectionIds.map((id) => {
        ...
        return (
          <button
            ...
            onClick={() => onSelect(id)}
            className={cn(
              "flex items-center shrink-0 rounded-[7px] border px-2.5 py-1.5 text-left text-[13px] transition-colors min-[900px]:w-full",
              isActive
