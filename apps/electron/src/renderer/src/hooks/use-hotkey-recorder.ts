...
export function isValidHotkeyCombo(combo: HotkeyCombo | null): boolean {
  ...
  // Dedicated function keys (F1 - F24) are standard dedicated hotkeys
  if (combo.key && /^F\d+$/i.test(combo.key)) return true;
  // Dedicated special keys: Insert, Pause, ScrollLock, PrintScreen
  if (
    combo.key &&
    ["Insert", "Pause", "ScrollLock", "PrintScreen"].includes(combo.key)
  ) {
    return true;
  }
  ...
  if (combo.key && combo.modifiers.length > 0) {
    const accel = combo.key
      ? [...combo.modifiers, combo.key].join("+")
      : combo.modifiers.join("+");
    if (BLOCKED_SHORTCUTS.has(accel)) return false;
    return true;
  }
  return false;
}
...
export function useHotkeyRecorder(
  ...
  options?: { target?: "dictation" | "magic-edit" | "custom" },
  ...
  const completeRecording = useCallback(() => {
    ...
    if (options?.target && options.target !== "dictation") {
      window.api?.stopHotkeyRecording();
    } else {
      // Re-register the main dictation listener with the new accelerator (single IPC)
