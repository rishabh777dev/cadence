...
import trayIconWinPath from "../../resources/tray/logoTemplate-win.png?asset";
...
process.on("uncaughtException", (err, origin) => {
  ...
  try {
    dialog.showMessageBoxSync({
      type: "error",
      title: "Cadence ran into a problem",
      message: "Cadence hit an unexpected error and needs to close.",
      detail:
...
let magicEditKeyListener: NativeKeyListener | null = null;
let currentGlobalShortcutFallback: string | null = null;
let currentMagicEditGlobalShortcut: string | null = null;
...
let magicEditPressed = false;
let currentMagicEditAccel: string | null = null;
  ...
  switch (channel) {
    ...
    case "toast": {
      ...
      if (message && Notification.isSupported()) {
        new Notification({ title: "Cadence", body: message }).show();
      }
      break;
    }
    case "navigate": {
...
function getFreestyleAppExclusions(): Set<string> {
  return new Set(
    ["Cadence", app.getName(), app.name, "Freestyle", "Electron"]
...
/**
 * After hiding the pill window, Windows needs time for the OS to transfer
 * input focus back to the previously-active application before paste
 * keystrokes will land there. 0ms (just yielding the microtask queue) is
 * insufficient — WM_ACTIVATE propagation takes 30–150ms on Windows. macOS
 * and Linux handle window-manager focus transitions synchronously within the
 * hide call, so they stay at 0ms.
 */
const HIDE_PILL_FOCUS_SETTLE_MS: Record<string, number> = {
  darwin: 0,
  win32: 50,
  linux: 0,
};
...
async function factoryReset(): Promise<void> {
  const { response } = await dialog.showMessageBox({
    ...
    message: "Delete all Cadence settings & data and restart?",
    detail:
...
function showMoveToApplicationsDialog(): void {
  ...
  dialog.showMessageBox({
    ...
    message:
      "Cadence is running from a read-only location and can\u2019t update itself.",
    detail:
      "Please drag Cadence into your Applications folder and relaunch it from there.",
    buttons: ["OK"],
  });
}
...
function createTray(): void {
  const chosenPath =
    process.platform === "win32" ? trayIconWinPath : trayIconPath;
  let trayImage = nativeImage.createFromPath(chosenPath);
  if (trayImage.isEmpty()) {
    const fallbacks = [
      join(app.getAppPath(), "resources/tray/logoTemplate-win.png"),
      join(__dirname, "../../resources/tray/logoTemplate-win.png"),
      join(process.resourcesPath, "resources/tray/logoTemplate-win.png"),
      join(app.getAppPath(), "resources/icon.png"),
      icon,
    ];
    for (const fb of fallbacks) {
      if (!fb) continue;
      const img = nativeImage.createFromPath(fb);
      if (!img.isEmpty()) {
        trayImage = img;
        break;
      }
...
const DEFAULT_MAGIC_EDIT_HOTKEY = getDefaultMagicEditHotkey();
...
function isValidAccelerator(accel: string): boolean {
  ...
  const modifiers = lowered.filter((p) => HOTKEY_MODIFIER_PARTS.has(p));
  const mouseButtons = lowered.filter((p) => HOTKEY_MACRO_MOUSE_PARTS.has(p));
  const regularKeys = lowered.filter(
    (p) => !HOTKEY_MODIFIER_PARTS.has(p) && !HOTKEY_MACRO_MOUSE_PARTS.has(p),
  );

  // Dedicated mouse buttons are valid on their own
  if (mouseButtons.length > 0) return true;
  // Dedicated function keys (F1-F24) or dedicated navigation/system keys (Insert, Pause, ScrollLock) are valid on their own
  if (
    regularKeys.length === 1 &&
    modifiers.length === 0 &&
    (/^f\d+$/i.test(regularKeys[0]) ||
      ["insert", "pause", "scrolllock", "printscreen"].includes(regularKeys[0]))
  ) {
    return true;
  }
  // A single bare modifier (e.g. "Control", "Alt", "Shift") is NOT valid —
  // this would fire on every ordinary keystroke and flood the app.
  if (regularKeys.length === 0 && modifiers.length <= 1) return false;
  // Two+ modifiers together (e.g. "Control+Alt", "Control+Shift") is valid
  if (regularKeys.length === 0 && modifiers.length >= 2) return true;
  // Modifier(s) + a regular key is valid
  if (regularKeys.length > 0 && modifiers.length > 0) return true;
  return false;
}
...
function notifyHotkeyDegraded(accel: string, nativeError: string): void {
  ...
  hotkeyLog.warn(body);
  if (Notification.isSupported()) {
    new Notification({ title: "Cadence is in toggle mode", body }).show();
  }
}
  ...
  if (process.platform === "linux") {
    if (isWaylandSession()) {
      ...
      hint = desktop.includes("gnome")
        ? " If a permission dialog appears on the next paste, allow Cadence to control input."
  ...
  for (const candidate of candidates) {
    try {
      if (globalShortcut.register(candidate, onToggle)) {
        ...
        currentGlobalShortcutFallback = candidate;
        return candidate;
      }
    } catch (err) {
      hotkeyLog.warn(
    ...
    const listener = new NativeKeyListener({
      ...
      onPermanentFailure: () => {
        ...
        if (registeredAccel) {
          ...
          const errorPayload = {
            message: `The hotkey listener stopped working and "${accel}" could not be re-registered. Restart Cadence or pick a different combination in Settings.`,
          };
...
function scheduleMagicEditHotkeyRegistration(hotkey?: string): void {
  void registerMagicEditHotkey(hotkey).catch((err) => {
    hotkeyLog.error(
      `Magic Edit hotkey registration failed: ${err instanceof Error ? err.message : String(err)}`,
    );
  });
}

function registerMagicEditGlobalShortcutFallback(accel: string): void {
  try {
    const onToggle = async (): Promise<void> => {
      if (!magicEditPressed) {
        ...
        magicEditPressed = false;
        mainWindow?.webContents.send("magic-edit:up");
        settingsWindow?.webContents.send("magic-edit:up");
      }
    };
    ...
    hotkeyLog.warn(
      `Failed to register Magic Edit global shortcut "${accel}": ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}

async function registerMagicEditHotkey(hotkey?: string): Promise<void> {
  try {
    if (magicEditKeyListener) {
      magicEditKeyListener.stop();
      magicEditKeyListener = null;
    ...
    if (!hotkey) {
      const settings = (await getServerSettings()) ?? {};
      hotkey = settings[SETTINGS_KEYS.magicEditHotkey];
    }

    let normalized: string;
    if (hotkey && isValidAccelerator(hotkey)) {
      normalized = normalizeAccelerator(hotkey);
    } else {
      if (hotkey) {
        hotkeyLog.warn(
          `Stored Magic Edit hotkey "${hotkey}" is invalid (bare modifier?), resetting to default "${DEFAULT_MAGIC_EDIT_HOTKEY}"`,
        );
    ...
    currentMagicEditAccel = normalized;

    let magicEditCapturePromise: Promise<void> | null = null;

    // Use NativeKeyListener first (handles push-to-talk & toggle, pure modifiers like Ctrl+Alt, mouse buttons, chords)
    const listener = new NativeKeyListener({
      hotkey: normalized,
      onKeyDown: () => {
        if (magicEditPressed) return;
        magicEditPressed = true;
        magicEditCapturePromise = (async () => {
          try {
            ...
            hotkeyLog.warn(`Magic edit capture failed: ${err}`);
          }
        })();
      },
      onKeyUp: () => {
        if (!magicEditPressed) return;
        magicEditPressed = false;
        if (magicEditCapturePromise) {
          void magicEditCapturePromise.then(() => {
            mainWindow?.webContents.send("magic-edit:up");
            settingsWindow?.webContents.send("magic-edit:up");
          });
          return;
      ...
      onPermanentFailure: () => {
        ...
        hotkeyLog.warn(
          "Magic Edit native key listener permanently failed; attempting Electron globalShortcut fallback.",
        );
        listener.stop();
        magicEditKeyListener = null;
        if (magicEditPressed) {
        ...
        registerMagicEditGlobalShortcutFallback(normalized);
      },
    });
    magicEditKeyListener = listener;

    const started = await listener.start();
    if (magicEditKeyListener !== listener) {
      listener.stop();
      return;
    }
    ...
    hotkeyLog.error(
      `registerMagicEditHotkey failed: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
