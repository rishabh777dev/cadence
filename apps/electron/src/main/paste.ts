...
async function pasteWindows(): Promise<"native" | "legacy"> {
  ...
  if (binaryPath) {
    try {
      ...
      log.warn(
        `windows-fast-paste exited with code ${exitCode}, falling back to PowerShell SendKeys`,
      );
    } catch (err) {
      log.warn("windows-fast-paste execution failed:", err);
    }
  }
  await execAsync(
...
async function copyWindows(): Promise<void> {
  const binaryPath = getNativeBinaryPath("windows-fast-paste");
  if (binaryPath) {
    try {
      const exitCode = await execFileAsync(binaryPath, ["--copy"]);
...
async function copyLinux(): Promise<void> {
  await tryExecAsync("xdotool key --clearmodifiers ctrl+c", "xdotool copy");
}
...
function restoreClipboardDirect(snapshot: ClipboardSnapshot): void {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    log.warn(`Failed to restore clipboard directly: ${message}`);
  }
}
...
function restoreClipboard(
  ...
  try {
    ...
    restoreClipboardDirect(snapshot);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
...
export async function captureSelectedText(): Promise<{
  text: string;
  restoreSnapshot: () => void;
}> {
  const prior = snapshotClipboard();
  const sentinel = `__cadence_capture_${Date.now()}__`;
  ...
  try {
    ...
    // Wait for the OS clipboard event loop to update with copied text
    for (let i = 0; i < 6; i++) {
      await new Promise((r) => setTimeout(r, 25));
      const currentText = clipboard.readText();
      if (currentText !== sentinel) {
    ...
    const message = err instanceof Error ? err.message : String(err);
    log.warn(`Failed to capture selected text: ${message}`);
  } finally {
    // If nothing was selected/copied, restore user's prior clipboard immediately
    // so screenshots and copied data are NEVER overwritten with sentinel strings.
    if (clipboard.readText() === sentinel) {
      restoreClipboardDirect(prior);
    }
  }
...
export function pasteIntoFocusedApp(
  ...
  const run = (): Promise<void> =>
    doPasteIntoFocusedApp(text, beforePaste, options?.appendSpace ?? true);
